...
- **Screenshots and a quick-tour GIF** in the README (and `README_zh.md`).
  Captured from the real UI booted against `scripts/seed_dev.py` data —
  not mockups. Lives in `docs/screenshots/`.
  - `tour.gif` — 8-second board → bugs → analytics walkthrough (~870 KB)
  - `board.png` — main board with three projects expanded
  - `bugs.png` — bug tracker showing the 7-status pipeline
  - `analytics.png` — burndown, bug trends, workload, blocker aging
