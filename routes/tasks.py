...
def create_task(t: TaskCreate, request: Request):
    ...
    with get_db() as conn:
        ...
        if feishu_notify:
            ...
            feishu_notify.notify_task_created(t.title_en, proj, ws_title, t.assignee, actor)
        return {"id": tid}
...
def update_task(tid: str, t: TaskUpdate, request: Request):
    ...
    with get_db() as conn:
        ...
        if feishu_notify and t.status and t.status != old_row["status"]:
            proj, ws_title = _notify_context(conn, old_row["workstream_id"])
            feishu_notify.notify_task_status_changed(
