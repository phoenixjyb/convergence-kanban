...
def test_done_chinese(bot, client, human_headers):
    ...
    # Bot can't mark done, but the command should resolve the ref and attempt it
    # It will error with 403 from governance — that's expected behavior
    result = bot.route_command(f"完成 {tid}", "test-human")
    # Either succeeds or hits governance error
    assert "done" in result.lower() or "Error" in result
